...
void calcDenseFlowGPU(string file_name, int bound, int type, int step, int dev_id,
                      vector<vector<uchar> >& output_xh,
                      vector<vector<uchar> >& output_xl,
                      vector<vector<uchar> >& output_yh,
                      vector<vector<uchar> >& output_yl,
                      vector<vector<uchar> >& output_img,
                      int new_width, int new_height){
    VideoCapture video_stream(file_name);
    CHECK(video_stream.isOpened())<<"Cannot open video stream \""
    ...
    while(true){
		...
		cnt ++;
            ...
            vector<uchar> str_xh, str_xl, str_yh, str_yl, str_img;
            encodeFlowMaps(flow_x, flow_y, str_xh, str_xl, str_yh, str_yl, bound);
            imencode(".jpg", capture_image, str_img);

            output_xh.push_back(str_xh);
            output_xl.push_back(str_xl);
            output_yh.push_back(str_yh);
            output_yl.push_back(str_yl);
            output_img.push_back(str_img);
