...
void calcDenseFlowGPU(string file_name, int bound, int type, int step, int dev_id,
                      ...
                      vector<vector<uchar> >& output_y,
                      vector<vector<uchar> >& output_img,
                      int new_width, int new_height, int stride, int if_reverse){
    VideoCapture video_stream(file_name);
    CHECK(video_stream.isOpened())<<"Cannot open video stream \""
                                  <<file_name
                                  <<"\" for optical flow extraction.";
    ...
    while(true){
        ...
        if (!initialized){
           ...
           if (capture_frame.empty()) return; // read frames until end
            ...
            cvtColor(prev_image, prev_gray, CV_BGR2GRAY);
            prev_gray.copyTo(prev_gray_array[0]);
            initialized = true;
            for(int s = 0; s < step; ++s){
                video_stream >> capture_frame;
		cnt ++;
            ...
            cvtColor(capture_image, capture_gray, CV_BGR2GRAY);
            stride_cnt++;
            if (stride_cnt < stride) {
              prev_gray_array[stride_cnt] = capture_gray;
              bool hasnext = true;
              for(int s = 0; s < step; ++s){
                  video_stream >> capture_frame;
	          cnt ++;
              ...
              capture_gray.copyTo(prev_gray_array[stride]); 
            }
            prev_gray_array[0].copyTo(prev_gray);
            if (if_reverse) {
              d_frame_0.upload(capture_gray);
              d_frame_1.upload(prev_gray);
            } else {
		...
		cnt ++;
                ...
                for (int ss = 0; ss < stride; ++ss) {
                  prev_gray_array[ss+1].copyTo(prev_gray_array[ss]);
                }
            }
